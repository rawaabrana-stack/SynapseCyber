...
// Your original <style> block, verbatim.
const CSS = `
  :root {
    --bg: #F8FAFC;
    --bg-alt: #F1F5F9;
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .footer-top { display: grid; grid-template-columns: 1.5fr repeat(4, 1fr); gap: 40px; padding-bottom: 56px; border-bottom: 1px solid #1E293B; }
  ...
  .socials a { width: 36px; height: 36px; background: #1c1917; border-radius: 6px; display: grid; place-items: center; font-size: 14px; transition: all 0.2s; }
  ...
  .a11y { border-top:1px solid #1E293B; margin-top:28px; padding-top:20px; }
...
// Your original <script> blocks, verbatim (nav scroll, hero replay, the four
// three.js scenes, scroll-reveal + count-up observers, EN/FR toggle).
// `THREE` is the global from the three.js r128 script (see note at bottom).
function runEffects() {
// =========================================================================
// NAV SCROLL - Switch nav from transparent to solid on scroll past hero
// =========================================================================
(function() {
...
// =========================================================================
// HERO REPLAY - Re-trigger intro animation whenever hero scrolls back into view
// =========================================================================
(function() {
...
// =========================================================================
// DOT GLOBE - Premium 3D scene: instanced dots, lighting, ambient particles
// =========================================================================
(function() {
  ...
  // ----- INSTANCED DOTS (proper 3D spheres, lit) -----
  const dotCount = 2400;
  const dotGeo = new THREE.SphereGeometry(0.028, 6, 6);
  const dotMatGray = new THREE.MeshStandardMaterial({
    color: 0x334155, roughness: 0.4, metalness: 0.2
  });
  ...
  // Pre-allocate: build two arrays first, then create InstancedMeshes
  const grayMatrices = [];
  const redMatrices = [];
  ...
  // ----- HOTSPOTS with emissive glow + pulsing rings -----
  const hotspots = [];
  const hotPoints = [
    { lat: 45, lon: -63 },   // Halifax
    { lat: 43, lon: -79 },   // Toronto
    { lat: 51, lon: 0 },     // London
  ...
  // ----- AMBIENT PARTICLES (drifting around globe in 3D space) -----
  ...
  const pMat = new THREE.PointsMaterial({
    size: 0.06,
    map: (function(){ var c=document.createElement('canvas'); c.width=c.height=64; var x=c.getContext('2d'); var g=x.createRadialGradient(32,32,0,32,32,32); g.addColorStop(0,'rgba(255,255,255,1)'); g.addColorStop(0.45,'rgba(255,255,255,0.85)'); g.addColorStop(1,'rgba(255,255,255,0)'); x.fillStyle=g; x.fillRect(0,0,64,64); return new THREE.CanvasTexture(c); })(),
    vertexColors: true,
    transparent: true,
    opacity: 0.7,
  ...
  let frame = 0;
  function animate() {
      if (window.__RM) { renderer.render(scene, camera); return; }
    frame++;
    const t = frame * 0.016; // ~60fps time
    ...
    // Shadow scales with bob (further = smaller shadow)
    ...
    // Subtle fill light orbit for moving highlight
...
// =========================================================================
// FOOTER SCENE - Subtle particle field
// =========================================================================
(function() {
  ...
  const mat = new THREE.PointsMaterial({ color: 0x60A5FA, size: 0.06, map: (function(){ var c=document.createElement('canvas'); c.width=c.height=64; var x=c.getContext('2d'); var g=x.createRadialGradient(32,32,0,32,32,32); g.addColorStop(0,'rgba(255,255,255,1)'); g.addColorStop(0.45,'rgba(255,255,255,0.85)'); g.addColorStop(1,'rgba(255,255,255,0)'); x.fillStyle=g; x.fillRect(0,0,64,64); return new THREE.CanvasTexture(c); })(), transparent: true, opacity: 0.85, depthWrite: false });
...
// =========================================================================
// SCROLL REVEAL - Replay fade-up animation each time element re-enters view
// =========================================================================
...
// =========================================================================
// COUNT-UP STATS - Reset and replay each time they re-enter view
// =========================================================================
  ...
  entries.forEach(entry => {
    ...
    if (entry.isIntersecting) {
      ...
      if (el.dataset.text) {
        ...
        if (window.__RM) { el.textContent = parseInt(el.dataset.count) + (el.dataset.suffix || ''); return; }
...
(function(){
  ...
  function setLang(lang){
    ...
    toggles.forEach(function(t){ t.classList.toggle('is-fr', lang === 'fr'); });
    try { localStorage.setItem('cx-lang', lang); } catch(e){}
  }
  var cur = 'en';
  try { if (localStorage.getItem('cx-lang') === 'fr') cur = 'fr'; } catch(e){}
  toggles.forEach(function(t){
    t.addEventListener('click', function(){ cur = (cur === 'fr' ? 'en' : 'fr'); setLang(cur); });
  });
...
<div className="topbar">
  <div className="container">
    ...
    <div><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#60A5FA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{verticalAlign: "-2px", marginRight: "5px"}}><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13.96.36 1.9.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.91.34 1.85.57 2.81.7A2 2 0 0 1 22 16.92z"/></svg>+1 902 555 0100 &nbsp;·&nbsp; <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#60A5FA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{verticalAlign: "-2px", marginRight: "5px"}}><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-10 5L2 7"/></svg>hello@synapsecyber.security</div>
  </div>
</div>

<nav>
  <div className="container">
    <a href="/" className="logo"><span className="logo-mark"></span>SynapseCyber</a>
    <div className="nav-links">
      <a href="#services" data-i18n="nav.services">Services</a>
      <a href="#industries" data-i18n="nav.industries">Industries</a>
      <a href="#methodology" data-i18n="nav.methodology">Methodology</a>
      <a href="#insights" data-i18n="nav.insights">Insights</a>
      <a href="#why" data-i18n="nav.why">Why Us</a>
      <a href="#faq" data-i18n="nav.faq">FAQ</a>
      <a href="#quote" className="nav-cta" data-i18n="nav.cta">Book a Consultation →</a>
...
<section className="hero-intro">
  ...
  <div className="hero-content">
    ...
    <h1 className="brand-title">
      <span style={{"--i": "0"}}>C</span><span style={{"--i": "1"}}>o</span><span style={{"--i": "2"}}>r</span><span style={{"--i": "3"}}>t</span><span style={{"--i": "4"}}>e</span><span style={{"--i": "5"}}>x</span><span style={{"--i": "6"}}>S</span><span style={{"--i": "7"}}>e</span><span style={{"--i": "8"}}>c</span>
    </h1>
    ...
    <div className="quote-panel reveal">
      ...
      <div className="quote-banner">
        ...
        <div className="quote-banner-inner">
          ...
          {/* Credentials panel (replaced the decorative graphic) */}
      ...
      <div className="quote-form-card">
        ...
        <div className="quote-form-row">
          <div className="form-field">
            ...
            <div className="field-wrap">
              ...
              <input type="text" placeholder="Company name" aria-label="Organisation" />
            </div>
          </div>
          <div className="form-field">
            ...
            <div className="field-wrap">
              ...
              <select aria-label="Service required">
        ...
        <div className="form-field">
          ...
          <textarea className="quote-textarea" aria-label="Project details" placeholder="Systems in scope, rough timeline, and any compliance drivers (PIPEDA, SOC 2, RCMP TRA)…"></textarea>
        </div>

        <button className="form-submit-pro">
        ...
        <div className="quote-card-footer">
          ...
          <span>Your information stays private, used only to prepare your quote. <a href="#">Privacy Policy</a></span>
        </div>
      </div>
  ...
  <div className="container">
    ...
    <div className="services-grid reveal-stagger">
      <div className="service-card">
        ...
        <a href="#" className="read-more">Read more <svg viewBox="0 0 12 10" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M1 5h10M7 1l4 4-4 4"/></svg></a>
      </div>
...
{/* ============ NOTABLE ENGAGEMENTS (replaces placeholder testimonials) ============ */}
      ...
      <div className="founder-right">
        ...
        <div className="founder-meta">
          ...
          <div className="founder-contact">
            <a href="mailto:hello@synapsecyber.security">hello@synapsecyber.security</a>
            <a href="#">LinkedIn &#8599;</a>
          </div>
        </div>
      </div>
    ...
    <div className="report-grid reveal">
      ...
      <div className="report-content">
        ...
        <form className="report-form" onSubmit={(e) => e.preventDefault()}>
          <input type="email" placeholder="Work email" className="report-input" />
          <button className="report-btn">Download sample <span className="arrow">→</span></button>
        </form>
        <p className="report-note">14-page PDF · No marketing follow-up</p>
      </div>
    </div>
  </div>
</section>
  ...
  <div className="container">
    <div className="cta-split reveal">
      ...
      <div className="cta-box">
        ...
        <a href="#" className="btn btn-secondary">Get the sample</a>
      </div>
      <div className="cta-box">
        ...
        <a href="#" className="btn btn-primary">Schedule now <span className="arrow">→</span></a>
      </div>
    </div>
  </div>
</section>
  ...
  <div className="container">
    <div className="footer-top">
      <div className="footer-brand">
        ...
        <div className="socials"><a href="#">in</a><a href="#">𝕏</a><a href="#">▶</a></div>
      ...
      <div className="footer-col"><h5 data-i18n="footer.services">Services</h5><ul><li><a href="#">Penetration Testing</a></li><li><a href="#">Threat & Risk Assessment</a></li><li><a href="#">Privacy Impact Assessment</a></li><li><a href="#">Red Team</a></li><li><a href="#">vCISO Advisory</a></li><li><a href="#">Incident Response</a></li></ul></div>
      <div className="footer-col"><h5 data-i18n="footer.industries">Industries</h5><ul><li><a href="#">Financial Services</a></li><li><a href="#">Healthcare</a></li><li><a href="#">Government & Crown Corps</a></li><li><a href="#">SaaS & Technology</a></li><li><a href="#">Critical Infrastructure</a></li></ul></div>
      <div className="footer-col"><h5 data-i18n="footer.resources">Resources</h5><ul><li><a href="#">Sample Pentest Report</a></li><li><a href="#">TRA Methodology Guide</a></li><li><a href="#">Blog & Research</a></li><li><a href="#">Case Studies</a></li></ul></div>
      <div className="footer-col"><h5 data-i18n="footer.company">Company</h5><ul><li><a href="#">About</a></li><li><a href="#">Careers</a></li><li><a href="#">Contact</a></li><li><a href="#quote" data-i18n="footer.quote">Request a Quote</a></li><li><a href="#">Privacy Policy</a></li><li><a href="#accessibility" data-i18n="footer.accessibility">Accessibility</a></li></ul></div>
    </div>
    <details className="a11y" id="accessibility">
      <summary data-i18n="footer.a11y">Accessibility · WCAG 2.1 AA / AODA</summary>
      <p>SynapseCyber is committed to digital accessibility. This site targets conformance with WCAG 2.1 Level AA and aligns with the Accessibility for Ontarians with Disabilities Act (AODA) and the Accessible Canada Act. It honours your operating system's reduced-motion preference, supports keyboard navigation, and meets AA colour-contrast ratios. If you encounter a barrier, email <a href="mailto:hello@synapsecyber.security">hello@synapsecyber.security</a> and we will respond promptly.</p>
    </details>
...
/*
  This page needs two things your original HTML loaded in <head>. Add them
  once to your app shell (e.g. Next.js app/layout or pages/_document):

  1. Fonts:
     <link href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,400;9..144,500;9..144,600;9..144,700&family=Manrope:wght@300;400;500;600;700;800&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet" />

  2. three.js r128 (the scenes use the global `THREE` and r128-only APIs):
